...
def extract_text_from_pdf(pdf_path, min_font_size, left_margin, right_margin):
    ...
    for page_num in range(len(document)):  # Iterate through all pages in the PDF
        ...
        for block in blocks:  # Iterate through each block of text
            if "lines" not in block:  # Check if 'lines' key exists in the block
                print(f"Warning: 'lines' key missing in block on page {page_num + 1}")
                continue  # Skip this block if 'lines' key is missing
...
scan_button.pack_forget()  # Hide the scan button initially
